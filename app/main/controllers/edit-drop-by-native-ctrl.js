'use strict';
angular.module('main')
    .controller('EditDropByNativeCtrl', function($scope, $stateParams, $global, $moment, SellNow, profile, $state, $timeout, $ionicLoading, $ionicScrollDelegate, $ionicSideMenuDelegate, $ionicPopup, $filter) {
        ...
        SellNow.getSellById(id).then(function(res) {
            ...
            setLocation(obj, $scope.data.agentId, true);
        })
        ...
        function setLocation(obj, agentId, init) {
            map.clear();
            map.off();
            // map.trigger("test");
            map.animateCamera({
        ...
        function getAgents(obj, agentId) {
            $ionicLoading.show();
        ...
        function plotAgents(agentId) {
            removeMarkers();
            var agents = angular.copy($scope.agents);
            if (agents.length) {
                for (var i = agents.length - 1; i >= 0; i--) {
                    if (agents[i].latitude && agents[i].longitude) {
                        if (agentId != undefined && agents[i].userId == agentId) {
                            $ionicLoading.hide();
                            $scope.vm.agent = agents[i];
                            $scope.data.agentId = agents[i].userId;
                            setPolyLine(agents[i]);
                        }
                        map.addMarker({
        ...
        $scope.sellNow = function() {
            ...
            if (!place && !consumerAddressId) {
                ...
                if (place) {
                    ...
                    SellNow.updateDrop($scope.data).then(function(res) {
                        if (res.status == $global.SUCCESS) {
